...
## Amendments after review (2026-09-05)

Recorded by the orchestrating session after a Fable critic pass; each item names what changed in the design above. The sections above are kept as written by the design pass; where they disagree with an amendment, the amendment wins.

1. **Two writers, one conformance bundle.** Python does not shell out to a Rust CLI for journal writes. music-hub gets a Python writer (`rfc8785` + `blake3` + append and fsync) and Ginger a Rust writer; both are pinned to the same conformance bundle in `music-hub/contracts/v0`. The bundle must also pin: on-disk line framing (canonical NDJSON bytes; readers re-canonicalize and compare), the idempotency comparison set (the intent body minus `recorded_at`), segment naming when one incarnation seals more than once (`<incarnation>-<seq-start>.ndjson`, never a bare `<incarnation>.open` collision), who recovers a crashed `.open` and when (the next incarnation of the same writer, at start, before writing), and the UUID version (v7, because UUIDs are an ordering key).
2. **Admission margin is measured, not fixed.** scsynth executes a late bundle immediately and prints a "late" line; timetags are output time, so a bundle must arrive at least one hardware buffer before its tag. Margin = measured device latency + one block + safety. scsynth's stderr is captured as evidence and the lag gate asserts zero "late" lines. Sub-block onsets use OffsetOut.
3. **Clock epoch versus execution epoch.** A clock discontinuity re-anchors at the next sealed boundary and writes a gap observation; it does not stop the instrument. Only a process restart starts a new execution epoch. A candidate whose `clock.bpm` differs from the running epoch is rejected with `unsupported_capability`; live tempo change joins the undecided list.
4. **Positions in captures come from the recording.** Rational tick-to-frame conversion yields idealized frames; capture-manifest sample positions are taken from the recording itself (marker impulses or the block counter) so an hour of ±50 ppm device drift cannot misplace a bar. The drift gate runs an hour, not a minute.
5. **Limits shed, they do not halt.** Periodic rule rates are bounded at `validate`; at runtime, exceeding a per-event or per-window limit sheds the lowest-priority work and writes an observation, rather than stopping scheduling.
6. **Replacement at boundary B.** The projected checkpoint is derived when the window B minus lookahead seals, not at prepare time, so late cues cannot leave the replacement initialized from stale state. Events emitted by old rules that cross B are dropped or remapped by authored id, and that is recorded. The old group stays alive until `/n_end` for all its voices; only its master send crossfades (100 ms). "Old voices finish their tails" and "100 ms crossfade" therefore both hold.
7. **NRT acceptance is measured, not byte-identical.** scsynth seeds its random generators from time unless `RandSeed` is set; the compiler emits a `RandSeed` per node from the declaration seed, and acceptance compares logical traces exactly and audio by measurement (onset, RMS, peak tolerances).
8. **JSON only in v0.** YAML is dropped from the first wave (the experiment and the v0 declaration are JSON). JCS: integer literals above 2^53 are forbidden by schema; `arbitrary_precision` is never enabled. Add `uuid` (v7) and a rational or i128 type to the crate list; pin `hound`; `rmcp` is post-week-two.
9. **Probe returns its own JSON.** `probe` reports peak, RMS, window bounds and evidence age as its own record; conversion to smpl feature frames is a separate later operation, not an acceptance criterion of the daemon.
10. **Audio ownership is decided for v0**, as section 6 says: one Ginger-owned scsynth, one stereo output, no inputs, capture inside the engine. The aggregate-device scenario on the plan page describes a later state.
11. **Canonical copy.** This file is the design of record; `astra-p8.out.md` in the vault's working directory is the source transcript.

### Amendments from the second review (Codex gpt-5.6-sol, verified against the installed SuperCollider 3.14.1 documentation and sources)

12. **Rules read only reducer-owned state.** A rule may read named state and declared control setpoints, never a value the engine computed. scsynth-side modulation (`/n_map`) is audio-only in v0; every control has one exclusive ownership mode: direct, mapped, or reducer-derived. If measured engine values are ever admitted as inputs, they enter as timestamped external cues and are replayed as observations in NRT.
13. **Executable intents are fsynced before dispatch**, and their terminal outcome before completion is acknowledged; only non-critical observations batch. Segment rotation and intent commitment are separate concerns. Startup recovers valid tails and rebuilds the nonce map before accepting requests.
14. **Clock discontinuity is a state machine, not a re-anchor.** scsynth may already hold up to a lookahead of bundles on the old clock. On a discontinuity: stop admission, mark the sealed interval unknown, `/clearSched`, rebuild pending events beyond a newly established safe boundary, verify with capture markers. If events may already have fired, start a new execution epoch; continuity is never claimed. (Amendment 3 is narrowed accordingly.)
15. **Randomness lives in Rust.** `RandSeed` per authored node is not independent randomness: overlapping voices reset and consume shared RNG slots in execution order (the installed RandSeed help warns about exactly this). v0 generates all stochastic parameters in the reducer. If a SynthDef must be stochastic, it carries `RandID.ir` plus `RandSeed.ir`, one slot per concurrently live voice, with the server's `-r` capacity sized to match and seeds derived from node plus voice ordinal.
16. **Replacement is restricted to restartable, event-driven operators in v0.** Named state survives; oscillator phase, sample cursors, envelopes, filters and delays do not. Operator manifests declare a release protocol, a maximum tail, forced-free behavior, and peak old-plus-new node, bus and RNG accounting. Feedback edges use exclusive buses and the explicit order reader, processing, writer; `InFeedback` alone does not establish it.
17. **Scheduling gets canonical definitions.** Tick-to-frame rounding and overflow limits are frozen; fixed half-open frame buckets anchored at epoch zero replace wake-relative windows so live and NRT shed the same losers; shedding drops whole causal events by a total order before reduction; conflicting writes have defined semantics and one linearization point for sealing; lookahead is at least the measured admission margin plus the wake and jitter allowance; cancellation is rejected once sealed. `OffsetOut` corrects `/s_new` onsets only.
18. **Evidence never overstates certainty.** Local transport is ordered TCP. Applied-ness is established by correlating `/fail`, `/n_go`, `/n_end`, post-boundary tree and node queries and `/c_get`, with `unknown` retained for races; `/g_queryTree` reports topology, SynthDef names and mapping symbols, not mapped-bus values. Node ids are generation-scoped. The master capture rotates into finalized chunks with frame-growth and channel-count validation; probe windows come from ping-pong `RecordBuf` buffers frozen before export. DiskOut channel mismatch and a crash before `/b_close` are treated as evidence failures.
19. **Hash stability across two languages.** Parse duplicate-aware raw JSON and hash the raw value before typed deserialization; enforce safe JSON numbers recursively; one bounded decimal-string grammar; timestamps exactly `...Z` with six fractional digits; lowercase UUID spelling; NFC or ASCII rules for identities; validate both record id and entry id on read. Order by explicit dependencies plus `(writer, incarnation, seq)`; UUIDv7 is opaque, not an order. Segments publish through a destination-side temporary file, a no-clobber rename, fsync of both directories, and a manifest carrying byte hash, sequence range and previous-segment hash. Ginger is the sole authority for executable intents unless a global nonce registry is added.
20. **NRT has a reproducible envelope.** Emit a canonical time-zero resource bundle and one ordered bundle per frame; length-prefix every score packet as SuperCollider's `Score` does; fingerprint scsynth version, architecture, SynthDef and asset bytes, UGens and all server options. Promise exact logical traces; promise measured audio equivalence only within a declared compatibility class, never cross-platform determinism.
21. **The two-day experiment crosses fewer seams.** Hard-code one graph and three SynthDefs, preload two muted continuations, switch with one cue and a crossfade, capture finalized chunks, emit a small comparison manifest. General replacement, ledger schemas and reusable lowering follow only if that loop proves valuable. Crate notes: add checked rational arithmetic and a timestamp library (`jiff` or `time`); order u64 text in SQLite by length then bytes; use scheme-specific hash constructors so ledger bytes and smpl PCM hashes cannot be confused.

**Freeze status after both reviews:** not ready to freeze. Blocked on reducer/readback ownership (12), intent durability (13), clock recovery (14) and RNG isolation (15); each is a wave-2 task. The contract task in wave 1 produces a draft and conformance vectors, not a frozen contract.
